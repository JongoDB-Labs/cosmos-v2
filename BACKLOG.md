...
**Status legend (marked by COSMOS Agent):** ✅ done & deployed (prod at 2.77.3) · 🟡 partial / verify · ⬜ open (not started). Version in _italics_ = where it shipped. _(Full implementation re-verification 2026-06-11: every item below cross-checked against the live codebase; markers updated to reflect what's actually built.)_
...
- ✅ **[feature] Bulk-edit issues (okr-dashboard parity).** _(done — all board types.)_ The org-wide **Issues view** + the board **Table view** had multi-select + bulk change; **2.73.0 added it to the Kanban board** too — a "Select" toggle enters a drag-disabled select mode (cards become checkbox toggles) with a bulk toolbar (move-to-column / priority / assignee / delete via `/work-items/bulk`, `ITEM_BULK_EDIT`/`ITEM_DELETE`-gated). The dnd-kit risk was isolated by giving the DndContext no sensors while selecting. Verified end-to-end (flat + swimlane).
...
- ✅ **[feature] Platform/sysadmin analytics + richer bug/FR telemetry.** _(done 2.70.0 + 2.70.1 — re-verified 2026-06-11.)_ All three layers are built: **(telemetry)** `src/lib/telemetry/breadcrumbs.ts` attaches app version + viewport + a console-breadcrumb trail to auto-captured client errors; **(analytics)** the Analytics → Feedback tab tracks reported/open/resolved bugs+FRs and shows a 🔁 hit-count·version badge on recurring auto-reported bugs (`analytics-dashboard.tsx`); **(attachments)** `feedback/attachments` route + portal upload/thumbnail let users attach screenshots to either ticket type. FR/BR voting is wired in the feedback portal.
...
- ✅ **[qa] Validate EVERY Settings subsection end-to-end from the frontend.** _(done — covered by the Settings-subsection audit item above + re-verified 2026-06-11; this is effectively a duplicate of it.)_ The parallel-agent audit confirmed all ~20 subsections are genuinely wired UI→route→DB. The specific **Security → SSO / sessions / IP allowlists** called out here are all present in `security-settings-panel.tsx`: `ssoEnforced` + `ssoConnectionId`, `sessionTimeoutMins` + active `SessionRecord` list, and `ipAllowlistEnabled` + CIDR `IpAllowlistEntry` management (session/IP controls shipped v4.20.0).
...
- ✅ **UUIDs leaking into the frontend and URLs instead of human-readable names.** _(done — re-verified 2026-06-11; the last open sub-item is closed.)_ The uuid→slug sweep (v4.18.0) handled mentions, slug-routed board/chat/template URLs with UUID→slug redirects, and ~18 raw-UUID label fallbacks. The remaining open sub-item — **project UUID in the meetings project-selector** — is fixed: the selector renders `{p.key} - {p.name}` and the 2.64.8 `collectSelectItems` flatten-fix makes `<Select.Value>` show that label app-wide (not the raw value). No UUIDs surface in the selector. Seen in multiple places — e.g. user UUIDs when tagging in comments, and project/board URLs use raw IDs (`/apex-defense/projects/SENTINEL/boards/4efcfc43-f68e-4d1f-b8d2-5351ff5b6288`). **Action:** do a thorough e2e sweep to ensure no UUIDs appear in the UI or in any URL — everything user-facing should resolve to a name/slug. Covers: visible labels (mentions, assignees, owners, etc.) and route segments (boards, and audit every other `[id]`-style route). Likely needs slug fields + lookups for entities that currently route by UUID. **Confirmed sightings (keep adding as found):** mention UUIDs in comments; board UUID in URL; **project UUID shown in a dropdown** (meetings project-selector). Do a clean full scrub of all frontend-facing UUIDs.
...
- ✅ **[bug] Not all roles available when creating a user.** _(resolved — not a bug; investigated 2026-06-10, re-confirmed 2026-06-11.)_ Both role selectors list the FULL enum; not reproducible as stated. Re-open ONLY with a specific repro (which flow + which role). The **invite-member** dropdown lists every assignable `OrgRole` — GUEST / VIEWER / MEMBER / BILLING_ADMIN / ADMIN — i.e. all 6 minus **OWNER**, which is deliberately excluded (ownership transfer is a separate flow); custom **work-roles** ARE assignable at invite time (checkbox list below the role). The **project-members** dropdown lists all 4 `ProjectRole`s (MANAGER / LEAD / MEMBER / VIEWER). So no standard role is missing from either selector. If a specific role is still absent, note WHICH flow + WHICH role (e.g. "expected OWNER in invite" → that's by design; "expected a custom work-role as the primary role" → those layer on top, not replace, the org role).
...
- ✅ **[bug] Board list doesn't refresh after create.** _(done — re-verified 2026-06-11.)_ Creating a board (`template-gallery.tsx`) `router.push`es straight into the new board, which re-renders the project layout server-side and re-fetches the board tabs — so the new board appears without a manual reload (it both shows AND navigates in, the ideal behavior the item asked for).

- ✅ **[bug/feature] No way to create issues.** _(done — re-verified 2026-06-11.)_ Per-column quick-create exists in the kanban/table/backlog views (`create-issue-button.tsx`), AND **2.74.0 added a full-field "New issue" dialog to the org-wide Issues view** (`CreateWorkItemDialog` wired into `issues-view.tsx` — title/project/type/priority/assignee/points/due/description/labels, `ITEM_CREATE`-gated).

- ✅ **[bug] Can't delete boards.** _(done — re-verified 2026-06-11.)_ Board delete is implemented in `board-tabs.tsx`: per-board delete (`boardToDelete`/`handleDeleteBoard`) with a confirmation Dialog + success toast, gated by `canManageBoards` (org `BOARD_DELETE` or project MANAGER), DELETE to the board route. The "wasn't added" note was stale.
...
- ✅ **[bug] COSMOS Agent drawer UI is rough.** _(done — re-verified 2026-06-11.)_ Moved to a bottom-right floating bubble overlay (v4.16.0), streaming scroll-jank fixed (2.65.1, instant stick-to-bottom only when pinned), history sidebar collapses by default (2.73.1). The 2026-06-11 desktop+mobile full-surface sweep found no layout collisions or console errors. Re-open only if a specific collision resurfaces.
...
- 🟡 **[feature] Mobile layout overhaul.** _(partial — core done; re-verified 2026-06-11: a 390px sweep of all 13 main pages found zero console/render errors. Only the persistent-search-BAR sliver remains open.)_ Rework the mobile experience:
...
- 🟡 **[feature] Full CRUD everywhere, gated by RBAC/ABAC.** _(partial — the named symptom-items are all now resolved; a formal end-to-end audit across every module is the remaining work.)_ The symptoms this item cited are fixed: board delete ✅, meeting CRUD (create/cancel/reschedule/delete) ✅, roles dropdowns ✅, Issues-view create ✅, bulk edit on all board types ✅. Remaining: a deliberate module-by-module CRUD audit (every entity has create/edit/delete, admin=full, user=RBAC/ABAC-gated) to close the long tail — not yet done end-to-end.

- 🟡 **[feature/convention] Right-click context menus throughout the app.** _(partial — broadly adopted, re-verified 2026-06-11.)_ A shared `ActionMenu` (right-click + ⋯ trigger, with scroll-jerk guard) is used across ~23 component files — issues table rows, kanban cards, board/list rows, CRM cards, etc. Remaining: make it a literal standing convention covering every remaining surface (nav, files) — codify in AGENTS.md. Not "everywhere" yet, but the mechanism + majority coverage exist.
