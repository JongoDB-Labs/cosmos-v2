...
- ⬜ **[ux, LOW] Create-board button hidden for VIEWER/GUEST-tier project MANAGERs.** The board API now lets a project MANAGER manage their project's boards even without org-wide `BOARD_*` (inheriting hierarchy, 2.55.0), and board edit/column/delete controls aren't client-gated so they work end-to-end. BUT the "create board" affordance in `issues-view.tsx` is gated by `can(Permission.BOARD_CREATE)` from the org-global permissions provider — so a MANAGER whose org role is VIEWER/GUEST (which lack `BOARD_CREATE`) won't see the button even though the API would allow it. MEMBER-tier managers (the common case) already hold org-wide `BOARD_CREATE` and are unaffected. Fix needs project-scoped manager state threaded into the client (the permissions provider is org-global today). Security-safe as-is (hides an allowed action; never grants a forbidden one).
