...
- 🟡 **[feature] Bulk-edit issues (okr-dashboard parity).** _(largely done — verified 2026-06-10.)_ The org-wide **Issues view** has multi-select + bulk change (status/assignee/priority/delete via `/work-items/bulk`, `ITEM_BULK_EDIT`-gated), AND the board **Table view** has the same (rowSelection + bulkUpdate, mirrors Issues). Remaining: multi-select on the **Kanban** board (selecting cards in a drag-and-drop surface) — deliberately DEFERRED (multi-select interacting with dnd-kit is genuinely risky to ship without extensive interaction testing; lower value than the table/issues paths which already cover bulk ops).
