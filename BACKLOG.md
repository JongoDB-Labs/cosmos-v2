...
**Status legend (marked by COSMOS Agent):** ✅ done & deployed (prod at 4.20.0) · 🟡 partial / verify · ⬜ open (not started). Version in _italics_ = where it shipped.

## Inbox (unsorted)

<!-- New items land here as they're called out. Triage into the sections below later. -->

- ⬜ **[security, HIGH] Request-time enforcement of org `mfaRequired` (the missing assurance gate).** `Session.mfaSatisfied` is written + surfaced by `getCurrentUser()` but NOTHING reads it to DENY a request — the org `mfaRequired` floor is only acted on via `revokeOrgSessions()` on the rising edge, and only for GOV tenants. So a non-enrolled user gets an `mfaSatisfied:false` session accepted everywhere, and a non-GOV org's `mfaRequired=true` does nothing. **This is a PRE-EXISTING platform gap (Google/SSO sessions are equally ungated at request time), surfaced by the password+MFA security review (2.41.0).** Needs a careful step-up gate: in `getAuthContext()` (and `proxy.ts` for org-scoped paths) load `OrgSecuritySettings.mfaRequired` and, when `mfaRequired && !mfaSatisfied && !internalAdmin`, redirect dashboard pages to a step-up/enroll screen + 403 org APIs — with the enroll path (Settings→Security + `/api/v1/me/mfa/*`) EXEMPT so users can't be locked out, and break-glass for `INTERNAL_ADMINS`. Deferred from 2.41.0 deliberately (lockout/redirect-loop risk warrants deliberate design, not a rushed change). Also add: a lockout warning before disabling MFA; optional TOTP replay-within-window rejection; derive client IP from the trusted proxy chain (not raw XFF).
...
- ⬜ **[bug] Backlog scroll escapes its container over the navbar.** Scrolling down the backlog while the cursor is hovered over the navbar scrolls the page "into nothingness" — the wheel scroll isn't captured by the backlog container when the pointer is over the nav. Ensure the scrollable backlog region owns the scroll (or the nav doesn't eat/redirect it).
- ⬜ **[bug] Classification banner + UUID-in-modal.** Per-project classification banner doesn't appear to be working on each project. The **Add Classification** window still shows the project **UUID instead of its name** in the project selector. **Action:** fix the per-project banner; and audit **EVERY modal/dialog/selector** to ensure users never see UUIDs instead of names/titles (extends the UUID-scrub item above).
- ⬜ **[feature] Fully configurable dashboard widgets.** Home dashboard widgets should be choosable from ANY page/section/data that exists in the app (whatever the user wants on their homepage), not just the current handful. Make the widget catalog driven by the real surface area.
...
- ⬜ **[qa] Connector end-to-end test + usage docs.** Test that each connector — **Google, GitHub, Jira, Slack, Microsoft 365** — actually works as intended, and document **how a user actually uses** each one (connect flow + what the agent/app can then do).
...
- ⬜ **[feature] Chat revamp → Teams/Mattermost-style.** Own messages on the right, everyone else on the left with names; delivered/read receipts; mobile-friendly messaging layout.
- ⬜ **[feature] Mobile navbar layout (everest-ai-style).** Chat in the CENTER of the mobile bottom nav; Overview + Projects to the left; Meetings + Notes to the right.
...
- ⬜ **[bug] Can't scroll on Project Settings.** _(open — not started.)_ The project settings page/panel can't be scrolled, so content below the fold is unreachable. Fix the scroll container / overflow.
...
- ⬜ **[bug] Can't delete boards.** _(open — board delete not yet added; create/rename/columns/saved-filter were.)_ There's no working way to delete a board. Add board deletion (with permission check + confirmation).
...
- ⬜ **[feature] Username/password login + MFA (alongside Google).** _(open — the Security work shipped the MFA POLICY as a flag-and-stub [no first-party TOTP enrollment yet]; full email/password auth + signup/reset + MFA enrollment/challenge is NOT built. Ties to "Security — granular controls".)_ Add email/password authentication with **MFA** (TOTP) as a login option **in addition to** Google sign-in.
