...
describe("buildRef", () => {
  ...
  it("builds refs like ACME-3", () => {
    expect(buildRef("ACME", 3)).toBe("ACME-3");
  });
...
describe("parseRef", () => {
  ...
  it("parses ACME-3 into key=ACME, number=3", () => {
    expect(parseRef("ACME-3")).toEqual({ key: "ACME", number: 3 });
  });
...
describe("round-trip: parseRef(buildRef(key, number))", () => {
  ...
  it("round-trips ACME and 3", () => {
    const built = buildRef("ACME", 3);
    const parsed = parseRef(built);
    expect(parsed).toEqual({ key: "ACME", number: 3 });
  });
