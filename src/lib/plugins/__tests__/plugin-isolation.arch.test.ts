...
describe("plugin isolation (ADR 0003)", () => {
  ...
  it("shared code never queries plugin-owned Prisma models (prisma.acme*)", () => {
    const offenders = sharedFiles.filter((rel) => {
      const text = readFileSync(join(ROOT, rel), "utf8");
      return /\bprisma\.acme[A-Z]/.test(text);
    });
    expect(
      offenders,
  ...
  it("the plugin framework itself stays brand/client-neutral", () => {
    ...
    for (const rel of FRAMEWORK) {
      const text = readFileSync(join(ROOT, rel), "utf8");
      // "Acme"/Acme may appear in the two composition files' import lines only —
      // the framework proper must not know any client's name.
      if (/Acme|Acme\b|Acme/i.test(text)) offenders.push(rel);
    }
    expect(
      offenders,
