 ...
 *
 * Atelier = Acme / Acme brand (Acme Brand Guide, Winter 2026): pearl canvas, deep
...
export const ATELIER_LIGHT_TOKENS: Record<string, string> = {
  ...
  "--laser": "#e9ff14", // Acme acid-yellow accent
  "--radius-sm": "2px",
  "--radius": "2px",
  "--radius-md": "2px",
  "--radius-lg": "4px",
  ...
  // at 48px — restrained but legibly "graph paper" (acme-atelier used 0.045).
...
// The Acme drafting-paper grid: a 48px midnight grid at low opacity (--atelier-grid).
const GRID_IMAGE =
 ...
 */
const ATELIER_CSS = [
  skinCss("acme", ATELIER_LIGHT_TOKENS),
  rule(":root.acme.acme.dark", ATELIER_DARK_TOKENS),
  // Inter's stylistic sets give atelier its refined type (Mabry Pro when licensed).
  `:root.acme { font-feature-settings: "ss01", "cv11", "cv05", "ss03"; font-variant-ligatures: contextual common-ligatures; }`,
  ...
  `:root.acme.acme body::before { content: ""; position: fixed; inset: 0; z-index: -2; background-color: var(--bg); background-image: none; pointer-events: none; }`,
  `:root.acme.acme body::after { content: none; }`,
  ...
  `:root.acme [data-app-canvas] { background-color: transparent; background-image: ${GRID_IMAGE}; background-size: 48px 48px; }`,
  // Selection reads as a quiet Acme accent.
  `:root.acme ::selection { background: var(--laser); color: #214144; }`,
].join("\n");
