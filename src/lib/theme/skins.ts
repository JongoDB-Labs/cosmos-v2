...
export const ATELIER_LIGHT_TOKENS: Record<string, string> = {
  ...
  // Drafting-paper grid line. acme-atelier used 0.045; bumped a touch so the
...
const ATELIER_CSS = [
  ...
  `:root.acme.acme body::before { content: ""; position: fixed; inset: 0; z-index: -2; background-color: var(--bg); background-image: ${GRID_IMAGE}; background-size: 48px 48px; pointer-events: none; }`,
  ...
  `:root.acme [data-app-canvas] { background-color: transparent; }`,
