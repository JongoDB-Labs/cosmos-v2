...
describe("atelier skin", () => {
  ...
  it("skinCss scopes tokens to the product root class", () => {
    const css = skinCss("acme", { "--bg": "#fff", "color-scheme": "light" });
    expect(css).toBe(":root.acme { --bg: #fff; color-scheme: light; }");
  });

  it("SKIN_CSS.atelier targets :root.acme with the pearl bg", () => {
    expect(SKIN_CSS.atelier).toContain(":root.acme {");
