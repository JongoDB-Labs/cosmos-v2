...
/**
 * Walk the Select's subtree and map each <SelectItem value> to its (string)
 * label. base-ui's <Select.Value> renders the raw value unless the Root is
 * given an `items` map; deriving it here means every call site shows labels
...
function SelectWithDerivedItems({
  items,
  children,
  ...props
}: SelectPrimitive.Root.Props<unknown>) {
  // Honor an explicit `items` map; otherwise derive one from the SelectItem
  // children so <Select.Value> shows labels instead of raw values everywhere.
...
// Cast back to base-ui's generic Root type so call sites keep inferring the
// value type (e.g. `onValueChange={(v) => …}` stays typed, not `any`).
const Select = SelectWithDerivedItems as unknown as typeof SelectPrimitive.Root
