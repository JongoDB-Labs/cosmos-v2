...
/**
 * The multi-select sibling of <SearchableSelect>: a dropdown with a built-in
 * filter input that lets you pick ANY number of options. Built on base-ui's
 * Combobox in `multiple` mode so the popup stays open while you toggle items and
 * the filter clears after each pick. Shares the single-select variant's visual
 * language, so search behaves consistently across dropdowns — use it wherever a
 * checkbox/multi dropdown would otherwise force users to scroll a long list
 * (e.g. assignees on a large org). (COSMOS-37)
 *
...
export function SearchableMultiSelect({
  ...
  emptyText?: string;
  /** How many selected labels to show in the trigger before collapsing to "+N". */
  maxLabels?: number;
  size?: "sm" | "default";
  ...
  // Control by the matching option objects (in `value` order) so base-ui's
  // referential equality (Object.is) lines up with the rendered items AND the
  // selection order the caller passed in is preserved. Map back to string ids at
  // the boundary so call sites stay string[]-in / string[]-out.
            ...
            vals.length === 0 ? (
              ...
              <span className="line-clamp-1 flex-1 text-left">
                {vals
                  ...
                  .map((v) => v.label)
                  .join(", ")}
                {vals.length > maxLabels ? ` +${vals.length - maxLabels}` : ""}
              </span>
            )
          }
        </Combobox.Value>
