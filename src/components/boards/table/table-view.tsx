...
export function TableView({ orgId, projectId, projectKey, boardId }: TableViewProps) {
  ...
  // Detail side panel (COSMOS-143, Open Task/Item in Table View — Phase 3):
  // open the same rich CardDetailSheet the Kanban board uses, so a table row can
  // be fully edited (description, status, assignee, priority, dates, comments…)
  ...
  const openItemById = useCallback(
    async (id: string) => {
      ...
      try {
        const full = await jsonFetch<WorkItem>(`${basePath}/work-items/${id}`);
        setDetailItem(full);
        setDetailOpen(true);
      } catch {
        ...
        cell: (info) => {
          ...
          return (
            <div className="flex w-full min-w-0 items-center gap-1.5">
              {/* Designated affordance (COSMOS-143): open the full detail panel.
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Detail side panel (COSMOS-143) — the same rich editor the Kanban board
          uses. Edits auto-save and flow back into the table row via
          handleItemUpdate; closing the sheet keeps saved changes. */}
      <CardDetailSheet
        item={detailItem}
        open={detailOpen}
