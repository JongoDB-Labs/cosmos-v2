...
/**
 * Notes as a DOCKED-DRAWER PANEL (body only — the DockedDrawer frame supplies
 * the tool tabs, resize, and close). A compact list of the org's notes and a
 * lightweight title + markdown-textarea editor (NOT the full Lexical
 * NoteEditor). Reads/writes the notes API:
...
export function NotesPanel({ orgId }: NotesPanelProps) {
  ...
  // Load once on mount (the panel mounts only while the Notes tool is active).
  // fetchNotes sets loading state synchronously — the established pattern — so
  // scope-disable the rule here.
