...
import { Settings, Send, Plus, ExternalLink } from "lucide-react";
...
/**
 * The streaming assistant chat as a DOCKED-DRAWER PANEL (body only — the
 * DockedDrawer frame supplies the tool tabs, resize, and close). A compact
 * bubble list, an autogrowing composer, and an INLINE settings panel (model +
 * active provider) toggled from this panel's own toolbar gear.
 *
 ...
 */
export function AssistantPanel({ orgId, orgSlug }: AssistantPanelProps) {
  ...
  // Abort any in-flight stream when the panel unmounts (drawer closed / tool
  // switched).
  useEffect(() => {
    return () => {
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Panel toolbar: New chat + settings gear (the global tabs/close live in
          the DockedDrawer header above this). */}
