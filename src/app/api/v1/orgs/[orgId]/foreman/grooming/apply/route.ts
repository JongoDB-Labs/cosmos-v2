...
import { applyGroomedVerdict } from "@/lib/foreman/grooming-actions";

/**
 * Apply a supervisor DRY proposal for real — the console's per-row "Apply" button.
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    return success(await applyGroomedVerdict(orgId, workItemId, ctx.userId));
  } catch (error) {
