...
| 13 | stale `ghcr.io` login breaks anonymous pulls | `zarf package create` → **`403 denied`** (not `401`) on a by-digest pull of a now-**public** image | `docker logout ghcr.io` so go-containerregistry pulls anonymously — don't "fix" it by depositing a broad-scope token on the build host |
| 14 | UDS doesn't auto-add OIDC egress for a `Package.sso` client | app login → `sso_discovery_failed`; ztunnel RSTs the Keycloak call under default-deny (silent at the netpol layer) | add an egress `allow` to the **tenant gateway** (the SSO-provider hop) in the Package — UDS creates the client, **not** the netpol |
