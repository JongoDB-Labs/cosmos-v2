...
- **`cosmos-pgbackrest-state` named volume** (mounted at `/var/lib/pgbackrest` on `cosmos-postgres` and `cosmos-backup`) — holds pgBackRest's stanza ledger **and** the `.stanza-ready` marker. This MUST be a named volume: if the marker lived in the container's writable layer it would vanish on any container recreate (image rebuild, `down`/`up`, `--force-recreate`, host reboot), `pgbackrest-archive-push.sh` would silently skip every WAL segment, and Postgres would recycle un-archived WAL → an unrecoverable PITR/RPO hole. As defense in depth, in steady state the wrapper also refuses to skip (pushes for real) if the marker is missing but pgBackRest's async spool dir for the stanza (`/var/spool/pgbackrest/archive/cosmos`) is present — i.e. real archiving has already run, so a missing marker is an anomaly, not pre-stanza bootstrap.
